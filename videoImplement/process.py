...
def process_single_trial(
    ...
    # Pass 1-4
    df, counts, _ = run_passes_1_to_4(df, config)

    # Pass 6
    df, _ = run_pass_6(df, config)
    ...
    graph.plotResults(df, savePath=out_plot, showPlot=show_plot, showMm=True)
    dprint(f"Saved plot: {out_plot}")

    # Final summary for this trial
    total = counts["total"]
    flagged_total = int(_flagged_mask(df).sum())
